...
As presented above, zk-SNARKs are successfully implemented in various application scenarios, there are still weaknesses to be discussed. The zero-knowledge proofs presented earlier are contrasted to condense differences and similarities. Ultimately, the main challenges of trust assumption, cost, and quantum computing are summarized in this chapter, providing an outlook on future solution approaches currently discussed in research.

\subsection{Assessment Approaches}
Zero-knowledge proof evaluation approaches (SOURCE GONG et al) can be grouped into computational complexity analysis \citep{LiuWangPengXing, SONIC10.1145/3319535.3339817, PipeZK}, security analysis \citep{Huangetal}, and communication cost \citep{ZHENG, liuetal}. This sub chapter is comparing specific aspects out of these categorizations. First, highlighted characteristics of zk-SNARKs, zk-STARKS, and Bulletproofs are revisited. Second, the analysis aspects of trusted setup, communication complexity, and quantum threat are emphasized. 

zk snarks:
- cryptographic assumption is secure bilinear pairings
-shorter proof length, smaller memory on-chain consumption, but trusted set up and crs
-trusted thrid party is to generate proving and verification key, how ever MPC can diffuse centralization of trust by letting hundreds of particpants generate the CRS

zk-starks: 
-faster verification and proof speed, although large number of circuits, no need for trusted setup and crs because of hash collision based symmetric encryption, free from arguments, which are vulnerable by future quantum computers
- more complex that snarks because of larger proof size (snarks is bytes, stark hundreds of KB)
%polylog : https://crypto.stackexchange.com/questions/48638/whats-the-difference-between-polylogarithmic-and-logarithmic

bulletproofs: 
-no trusted setup, logarithmically increasing proof size, good applicability with range proofs, they can save internal storage, well suitable for short proof sizes for general arithmetic circuits
- bulletproofs enable storing proofs on-chain easily, which is different to all other ZKPs
...
\subsection{Future of Zero-Knowledge Proofs}
